...
export function ClosingCTA({ eyebrow = "Next step", title, body, primary, secondary, tertiary, variant = "dark" }: {
  ...
  return (
    <section className={cn(styles.closing, "py-20 sm:py-28 lg:py-32", dark ? "bg-[#081540] text-white" : "border-t border-line bg-sunken text-ink")}>
      {dark && <div aria-hidden className={styles.orbits} />}
      <Container>
        <Reveal>
          <div className="max-w-4xl">
            <p className={cn("eyebrow mb-6 flex items-center gap-2.5", dark ? "text-[#8d98b8]" : "text-faint")}><span aria-hidden className="h-[5px] w-[5px] bg-[var(--brand-blue)]" />{eyebrow}</p>
