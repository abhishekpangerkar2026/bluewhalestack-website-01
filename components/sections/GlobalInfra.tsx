...
export function GlobalInfra({ content, regions }: { content: HomeExtras["global"]; regions: SiteSettings["regions"] }) {
  return (
    <section id="global-infrastructure" className={`${styles.global} py-20 sm:py-28`}>
      <Container className="relative">
        <div className="grid items-start gap-12 lg:grid-cols-[1fr_1.05fr] lg:gap-24">
          <Reveal>
            <SectionHeading inverse {...content.heading} />
            <div aria-hidden className="mt-10 flex items-center gap-4 border-t border-white/10 pt-6">
              ...
              <div className="flex flex-wrap items-center gap-x-4 gap-y-2 font-mono text-[10px] tracking-[0.12em] text-slate-400">
                {regions.map((region, index) => <span key={region.code} className="inline-flex items-center gap-4">{region.code}{index < regions.length - 1 && <span className="h-px w-5 bg-white/20" />}</span>)}
              </div>
            </div>
          </Reveal>
          <Reveal delay={100}>
            <div className="mb-2 flex justify-between pl-7 text-[10px] font-medium uppercase tracking-[0.16em] text-slate-400"><span>{content.directoryLabel}</span><span>{String(regions.length).padStart(2, "0")} {content.locationsLabel}</span></div>
        ...
        <div className="mt-14 grid grid-cols-2 gap-x-8 gap-y-10 border-t border-white/15 pt-9 sm:mt-20 lg:grid-cols-4">
          {content.stats.map((stat) => <Stat key={stat.label} value={stat.value} label={stat.label} inverse />)}
        </div>
      </Container>
    </section>
  );
