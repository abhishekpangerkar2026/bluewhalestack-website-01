...
export const homePageQuery = `*[_type == "homePage" && _id == "homePage"][0]{
  badge, title, titleAccent, description, primaryCta{label, href}, primaryNote, secondaryCta{label, href},
  ...
  products[]{name, role, body, status, href, ${img}}
}`;

export const pageHeroQuery = `*[_type == "pageHero" && route == $route][0]{
  route, eyebrow, title, titleAccent, description, ${img}, ${videoProjection}
}`;

export const editionsQuery = `*[_type == "edition"] | order(order asc, _createdAt asc){
  ${slug}, name, badge, tagline, headline, positioning, summary, audience, outcome, fits, buyWhen,
...
export const modulesQuery = `*[_type == "module"] | order(order asc, _createdAt asc){
  ${slug}, name, group, icon, tagline, description, features, ${img},
...
export const industriesQuery = `*[_type == "industry"] | order(order asc, _createdAt asc){
  ${slug}, name, icon, title, description, ${img}, edition, outcome, story, architectureId,
...
export const solutionsQuery = `*[_type == "solution"] | order(order asc, _createdAt asc){
  ${slug}, name, icon, summary, description, ${img}, problem, symptoms, features, ${facts("facts")},
