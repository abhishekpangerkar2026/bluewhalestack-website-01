...
import { editionsPage } from "@/content/sections/editionsPage";
...
const getContent = () => getPageDoc(editionsPageSpec, editionsPage);

export async function generateMetadata(): Promise<Metadata> {
  const builtMeta = await builtMetadata("/editions");
...
export default async function EditionsPage() {
  const built = await builtPage("/editions");
              ...
              <Reveal key={e.slug} delay={(i % 4) * 70}>
                ...
                >
                  <div className="flex items-center justify-between">
                    {e.comingSoon ? (
                      <Badge tone="neutral">
                        {c.cards.previewLabel}{e.gaTarget ? ` · GA ${e.gaTarget}` : ""}
                      </Badge>
                    ) : e.featured ? (
                      <Badge tone="warning">{c.cards.featuredLabel}</Badge>
                    ) : (
                  ...
                  <div className="mt-4 flex-1 space-y-4">
                    <div>
                      <p className="eyebrow">{c.cards.audienceLabel}</p>
                      <p className="mt-1 text-sm leading-relaxed text-muted">{e.audience}</p>
                    </div>
                    <div>
                      <p className="eyebrow">{c.cards.includesLabel}</p>
                    ...
                    <div>
                      <p className="eyebrow">{c.cards.outcomeLabel}</p>
                  ...
                  <dl className="mt-5 space-y-1.5 border-t border-line pt-5 text-sm">
                    <div className="flex justify-between gap-3">
                      <dt className="text-faint">{c.cards.deployLabel}</dt>
                      <dd className="text-right font-medium text-ink">
                        {e.deploy.join(" · ")}
                      </dd>
                    </div>
                    <div className="flex justify-between gap-3">
                      <dt className="text-faint">{c.cards.priceLabel}</dt>
                      <dd className="text-right font-medium text-ink">
                        {e.priceAnchor}
                      </dd>
                    </div>
                  ...
                  >
                    {c.cards.exploreLabel} {e.name} <ArrowRight className="h-3.5 w-3.5" />
                  </Link>
                </Card>
              </Reveal>
            ))}
